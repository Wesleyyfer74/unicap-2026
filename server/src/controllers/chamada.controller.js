...
export async function list(request, response, next) { try { return response.json(await chamadaService.list(request.validated.query)); } catch (error) { return next(error); } }
...
export async function create(request, response, next) { try { return response.status(201).json({ chamada: await chamadaService.create(request.validated.body) }); } catch (error) { return next(error); } }
