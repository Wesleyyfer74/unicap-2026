...
export async function create(request, response, next) { try { return response.status(201).json({ aluno: await alunoService.create(request.validated.body) }); } catch (error) { return next(error); } }
...
export async function update(request, response, next) { try { return response.json({ aluno: await alunoService.update(request.validated.params.id, request.validated.body) }); } catch (error) { return next(error); } }
