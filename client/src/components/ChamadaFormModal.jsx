...
export default function ChamadaFormModal({ saving, onSave, onClose, chamada = null }) {
  const { administrador } = useAuth();
  const [fiscais, setFiscais] = useState([]);
  const [form, setForm] = useState({ fiscalId: chamada ? String(chamada.fiscalId) : '', turno: chamada?.turno || TRANSPORT_SHIFTS[0].value, corOnibus: chamada?.corOnibus || 'Azul' });
  ...
  return <Modal title={chamada ? 'Editar chamada' : 'Nova Chamada'} onClose={onClose}><form className="modal-form" onSubmit={submit}>
    ...
    <footer><button type="button" className="button button-secondary" onClick={onClose}>Cancelar</button><button className="button button-primary" disabled={saving}>{saving ? 'Salvando...' : chamada ? 'Salvar alterações' : 'Iniciar chamada'}</button></footer>
  </form></Modal>;
}
