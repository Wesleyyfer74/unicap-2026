...
export default function ChamadaFormModal({ saving, onSave, onClose }) {
  ...
  return <Modal title="Nova Chamada" onClose={onClose}><form className="modal-form" onSubmit={submit}>
    ...
    <label htmlFor="call-shift">Turno</label><select id="call-shift" value={form.turno} onChange={(event) => setForm({ ...form, turno: event.target.value })}><option value="MATUTINO">Matutino</option><option value="INTEGRAL">Integral</option><option value="NOTURNO">Noturno</option></select>
